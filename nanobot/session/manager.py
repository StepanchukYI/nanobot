...
class Session:
    ...
    def _is_history_slice_coherent(messages: list[dict[str, Any]]) -> bool:
        """Validate tool-call/tool-result adjacency so we never send orphaned tool results."""
        ...
        for m in messages:
            ...
            if role == "tool":
                ...
                if not tool_call_id or tool_call_id not in declared_ids:
                    return False
    ...
    def get_history(self, max_messages: int = 500) -> list[dict[str, Any]]:
        ...
        sliced = self._trim_to_coherent_history(unconsolidated[-max_messages:])
